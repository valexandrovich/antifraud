...
function DropzoneJuridical() {
  ...
  const fileID = useSelector((state) => state.auth.juridicalFileID);
  useEffect(() => {
    const resetFile = () => {
      if (fileID != null) {
        fetch(`/api/uniPF/getUploadedJuridical/${fileID}`, {
          headers: authHeader(),
        })
          .then((res) => res.json())
  ...
  const uploadFileExample = () => {
    fetch("/api/uniPF/downloadJuridicalFile", {
      headers: authHeader(),
    })
      ...
      .then((blob) => {
        ...
        link.href = url;
        link.download = "ExampleJuridicalFile.xlsx";
        link.click();
      });
  };
  ...
  const updateErrors = async (id, index, value) => {
    ...
    await fetch(
      `/api/uniPF/updateJuridical?id=${id}&index=${index}&value=${value}`,
      requestOptions
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(
      `/api/uniPF/uploadJuridical?delimiter=${csvoptions.delimeter}&code=${csvoptions.codingType}`,
      {
        headers: authHeader(),
        method: "POST",
        body: formData,
      ...
      .then(setCsv(null))
      .then((result) => {
        dispatch(setJuridicalFileID(result));
        fetch(`/api/uniPF/getUploadedJuridical/${result}`, {
          headers: authHeader(),
        })
          .then((res) => res.json())
          .then((file) => setPrevFile(file))
  ...
  const downloadwithDescription = (id, value) => {
    ...
    fetch(
      ...
      if (res.status === 200) {
        ...
        dispatch(setJuridicalFileID(null));
        setPrevFile([]);
      }
      if (res.status > 300) {
    ...
    <>
      ...
      <div className="col-sm-12 mt-2">
        <button
          disabled={
            description.trim() === "" ||
            prevFile?.statusListCompany?.length > 0 ||
            prevFile?.statusListTag?.length > 0 ||
            !fileID
          }
          onClick={() =>
            downloadwithDescription(prevFile.companies[0].uuid, description)
          }
      ...
      {prevFile.companies && prevFile.companies.length > 0 && (
        <Table
          canEdit={true}
          data={prevFile.companies}
          err={prevFile.statusListCompany}
...
export default DropzoneJuridical;
