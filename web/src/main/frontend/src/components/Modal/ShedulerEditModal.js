...
const ShedulerEditModal = ({open, onClose, edit, groupName, exchange}) => {
    ...
    useEffect(() => {
        const selectedPeriod = () => {
            if (
                ...
                setPeriod("weeks");
            } else if (rowDate.schedule?.month &&
            ...
            if (rowDate.schedule?.days_of_week) {
                ...
                const notFiltered = daysArray.filter(([_key, value]) => value !== "all");
    ...
    const handleValidate = (e) => {
        ...
        const minutes = () => {
            ...
            if (typeof time.periodic.value == "string" && minPeriod === "periodic" && time.periodic.value.indexOf("h") > 0) {
        ...
        const dayOfMonth = () => {
            ...
            if (period === "month" &&
                ...
                return rowDate.schedule.days_of_month.value === undefined ? undefined : rowDate.schedule.days_of_month;
            }
            if (
                period === "month" &&
                monthPeriod === "daymonth" &&
        ...
        const daysOfWeek = () => {
            let dw = period === "month" && monthPeriod === "dayweek" ? weekDays?.month : (period === "weeks" ? weekDays?.weeks : null);

            if (!dw || (Object.values(dw).every(isAll) && Object.keys(dw).length === 7) || Object.keys(dw).length === 0) {
                return undefined;
            }
            return dw;
        };
