...
const Table = (props) => {
  ...
  const getRowsData = () => {
    ...
    return props.data.map((row, index) => {
      ...
      return (
        <tr key={row.id}>
          <RenderRow key={index} data={row} keys={keys} err={errorRow} eTag={props.errTag} id={row.id} updateErrors={props.updateErrors} />
        </tr>
      );
    });
  };
...
const RenderRow = (props) => {
  return props.keys.map((key, index) => {
    ...
    return err ? (
      <ErrCell updateErrors={props.updateErrors} idx={index - 2} key={index} data={props.data[key]} mis={err} id={props.id} />
    ) : (
      <td key={index}>{typeof props.data[key] === "object" && props.data[key] !== null ?
        <Tag updateErrors={props.updateErrors} errTag={props.eTag} data={props.data[key]} /> : props.data[key]}</td>
    );
  });
};
      ...
      {edit ? (
        <div>
          ...
          <div className="d-flex justify-content-between mt-1">
            <button onClick={() => props.updateErrors(id, idx, errValue)} type="button" className="btn custom-btn  btn-default">
              Змінити
            </button>
            ...
            <div key={el.id} className="tags d-flex border-bottom">
              {Object.entries(el).slice(1).map(([k, v], index) => {
                ...
                if (errCell?.columnIndex + 1 === index) {
                  return (<>{<ErrCell updateErrors={updateErrors} key={index} idx={index} id={el.id} data={v} mis={errCell.columnIndex + 1 === index && errCell} name={k} />}</>);
                }
                return (<div className="d-flex flex-column align-items-center" key={index}> <span className="table-header">{k.replace("mk", "")}</span><span>{v} </span></div>);
              }
              )}
            </div>
          );
