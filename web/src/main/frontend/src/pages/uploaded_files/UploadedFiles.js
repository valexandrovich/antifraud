...
const UploadedFiles = () => {
  ...
  const deleteAction = (id) => {
    ...
    fetch(`/api/uniPF/delete/${id}`, requestOptions)
      .then((res) => {
        if (res.status === 200) {
          dispatch(
            setAlertMessageThunk(
              `Видалено запис ${id}`, "success"
            )
          );
          getFiles();
  ...
  const enrich = (id) => {
    ...
    fetch(`/api/uniPF/enricher/${id}`, requestOptions)
      .then((res) => {
        if (res.status === 200) {
          dispatch(
            setAlertMessageThunk(
              `Завантажено запис ${id}`, "success"
            )
          );
          getFiles();
          setConfirmationRemove(null);
  ...
  return (
    <div className="wrapped">
      ...
      <div className="sroll-x">
                ...
                <tr key={el.uuid}>
                  ...
                  <td>
                    <div className="d-flex align-items-center">
                      <button onClick={() => enrich(el.uuid)} className="btn custom-btn">Завантажити</button>
                      <span onClick={() => setConfirmationRemove(el.uuid)} style={{ color: "red", fontSize: "30px" }}><IoIcons.IoMdTrash className="mr-10" /></span>
                    </div>
                  </td>
        ...
        {confirmationRemove && <ConfirmDeletemodal
          open
      ...
      {singleFile.persons && singleFile.persons.length > 0 && (
        <Table data={singleFile.persons} err={singleFile.statusListPerson} errTag={singleFile.statusListTag} />
      )}
    </div>
  );
};
